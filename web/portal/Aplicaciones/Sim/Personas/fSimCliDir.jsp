...
<Portal:Pagina funcion="SimClienteDireccion">
	<Portal:PaginaNombre titulo="Direcci&oacute;n" subtitulo="Modificaci&oacute;n de datos" subtituloalta="Alta de datos"/>

	<Portal:Forma tipo='catalogo' funcion='SimClienteDireccion' parametros='IdPersona=${param.IdPersona}&IdDomicilio=${param.IdDomicilio}&IdExConyuge=${param.IdExConyuge}&DomicilioFiscal=${param.DomicilioFiscal}'>
	
		<Portal:FormaSeparador nombre="Direcci&oacute;n del Cliente"/>				
		...
		<input type="hidden" name="TipoAsentamiento" value='<c:out value='${requestScope.registro.campos["TIPO_ASENTAMIENTO"]}' />' />
			...
			<th>Domicilio fiscal</th>
			    <td> 
				<input type='checkbox' name='DomicilioFiscalSi'/ checked >S&iacute
			    </td>
			</tr>
	    	</c:if>
	    	
	    	<c:if test='${param.DomicilioFiscal == "F"}'>
		    	<c:if test='${(requestScope.registro == null)}'>
				<tr> 
				<th>Domicilio fiscal</th>
				    <td> 
					<input type='checkbox' name='DomicilioFiscalSi'/>S&iacute
				    </td>
				</tr>
			</c:if>
		    	<c:if test='${requestScope.registro.campos["DOMICILIO_FISCAL"] == "V"}'>
