...
<Portal:Pagina funcion="GeneralesUsuario"  precarga="GeneralesUsuarioPerfil">
	<Portal:PaginaNombre titulo="Usuario" subtitulo="Modificaci&oacuten de datos" subtituloalta="Alta de datos"/>
	
	<Portal:Forma tipo='catalogo' funcion='GeneralesUsuario'>
		...
		<Portal:FormaElemento etiqueta='' control='etiqueta' controlvalor=''>		
			<Portal:Url tipo='catalogo-ventana' url='/ProcesaCatalogo?Funcion=GeneralesPersona&OperacionCatalogo=IN&Filtro=Todos&CveGpoEmpresa=${param.CveGpoEmpresa}' nombreliga='Persona' nomventana='VentanaPersona'/>
		</Portal:FormaElemento>

		<c:if test='${(requestScope.registro != null)}'>
		<Portal:FormaElemento etiqueta='' control='etiqueta' controlvalor=''>		
		    <a id="DatosPersona" href="javascript:MM_openBrWindow('/portal/ProcesaCatalogo?Funcion=GeneralesPersona&OperacionCatalogo=CR&Consulta=SI&IdPersona=&Ventana=Si','VentanaCp','scrollbars=yes,resizable=yes,width=700,height=400') ">Consulta datos de la persona</a>			
		</Portal:FormaElemento>
		</c:if>	
	...
	<Portal:TablaForma maestrodetallefuncion="GeneralesUsuarioPerfil" nombre="Aplicaciones asignadas al usuario" funcion="GeneralesUsuarioPerfil" operacion="BA" parametros='CveUsuario=${registro.campos["CVE_USUARIO"]}&CveGpoEmpresa=${registro.campos["CVE_GPO_EMPRESA"]}' >
		...
		<Portal:FormaBotones>
			...
			<Portal:Boton tipo='catalogo' etiqueta='Agregar nuevas aplicaciones' funcion='GeneralesUsuarioPerfil' operacion='CT' filtro="PerfilesDisponibles" parametros='&CveUsuario=${requestScope.registro.campos["CVE_USUARIO"]}&CveGpoEmpresa=${param.CveGpoEmpresa}'/>
