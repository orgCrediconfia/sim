 ...
 * modificaci�n y consulta) del cat�logo de publicaciones. Esta clase es llamada por
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			if (registroPublicacion == null){
				...
				registroControl.sPagina = "/Portales/Sim/CrediConfia/SistemaErrorPagina.jsp?Mensaje=Su perfil no tiene acceso a la secci�n de la publicaci�n";
			}
			else {
			...
			if (request.getParameter("Filtro").equals("Alta")){
				...
				if (registroUsuario == null){
					...
					registroControl.sPagina = "/Portales/Sim/CrediConfia/SistemaErrorPagina.jsp?Mensaje=El usuario no esta dado de alta como usuario de publicaciones y no tiene asignado un perfil de publicaci�n";
				}
				else {
					...
					if (sCvePerfilPub == null){
						...
						registroControl.sPagina = "/Portales/Sim/CrediConfia/SistemaErrorPagina.jsp?Mensaje=El usuario no tiene asignado un perfil de publicaci�n";
					}
					else {
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (sNombreArchivo.equals("")){
			...
			if(registro.getDefCampo("OPERACION_CATALOGO").equals("BA") ){				
				...
				//VALIDA QUE EL TAMA�O DEL ARCHIVO IMAGEN NO SEA MAYOR A 10 KB
				if (iTamanoArchivoImagen > 11000){
					com.rapidsist.portal.catalogos.ResultadoCatalogo resultadoCatalogoControlado = new com.rapidsist.portal.catalogos.ResultadoCatalogo();
					resultadoCatalogoControlado.mensaje.setClave("PUBLICACION_IMAGEN_TAMANO");
					resultadoCatalogoControlado.mensaje.setTipo("Error");
					resultadoCatalogoControlado.mensaje.setDescripcion("El tama�o del archivo imagen debe ser menor a 10 KB");
					registroControl.resultadoCatalogo = resultadoCatalogoControlado;
				}
				else{
					...
					//ACTUALIZA EL REGISTRO EN LA BASE DE DATOS
						...
						// LA OPERACION CATALOGO ES MODIFICACI�N.
						registroControl.resultadoCatalogo = catalogoSL.modificacion("PublicacionesSolicitud", registro, iTipoOperacion);
