 ...
 * modificación y consulta) del catálogo de secciones asociadas a un perfil. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class PublPerfilSeccionCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			if (!request.getParameter("NomSeccion").equals("")){
				parametros.addDefCampo("NOM_SECCION", request.getParameter("NomSeccion"));
			}
			registroControl.respuesta.addDefCampo("ListaBusqueda", catalogoSL.getRegistros("PublicacionesPerfilSeccion", parametros));
			registroControl.sPagina = "/Aplicaciones/Publicaciones/fPublPerfilAlt.jsp";
		}
		else if (iTipoOperacion == CON_CONSULTA_REGISTRO){
			//OBTIENE SOLO EL REGISTRO SOLICITADO
			//parametros.addDefCampo("CVE_GPO_RESP",request.getParameter("CveGpoResp"));
			//registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("PublicacionesGrupoResponsableSeccion", parametros));
			//registroControl.sPagina = "/Aplicaciones/Publicaciones/fPublGrupoResponsableAlt.jsp";
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		//RECUPERA LA LISTA DE LAS SECCIONES QUE SE VAN A PROCESAR
		LinkedList listaSecciones = null;
		String sCveSeccion;
		...
		while (lista.hasMoreElements()){
			...
			//VERIFICA SI EL PARAMETRO TIENE EL PREFIJO "SeccionAlta"
			if (sNombre.startsWith("SeccionAlta")){
				//VERIFICA SI LA LISTA DE FUNCIONES ESTA INICIALIZADA
				if (listaSecciones == null){
					listaSecciones = new LinkedList();
				}
				sCveSeccion = sNombre.substring(11, sNombre.length());
				listaSecciones.add(sCveSeccion);
			}
		}
        //VERIFICA SI SE SELECCIONARON FUNCIONES PARA PROCESAR
		if (listaSecciones != null){
			registro.addDefCampo("ListaPerfilSeccion", listaSecciones);
			//DA DE ALTA O BAJA LAS FUNCIONES EN LA BASE DE DATOS
			registroControl.resultadoCatalogo = catalogoSL.modificacion("PublicacionesPerfilSeccion", registro, iTipoOperacion);
		}
		else{
		...
		registroControl.sPagina = "/ProcesaCatalogo?Funcion=PublicacionesPerfil&OperacionCatalogo=CR&CvePerfilPub=" + request.getParameter("CvePerfilPub");
		return registroControl;
	}
}
