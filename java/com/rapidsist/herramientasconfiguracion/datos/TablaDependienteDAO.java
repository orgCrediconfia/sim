...
/**
 * Administra los accesos a la base de datos para el catálogo de tablas relacionadas ó dependientes.
 */
public class TablaDependienteDAO extends Conexion2 implements OperacionConsultaTabla  {
		...
		if (sFiltro.equals("TablasDisponibles")){
			sSql = "SELECT * FROM RS_CONF_TABLA \n" +
					" WHERE CVE_TABLA NOT IN ( \n" +
							  " SELECT CVE_TABLA_DEPENDIENTE FROM RS_CONF_TABLA_DEPENDIENTE \n"+
							  "   WHERE CVE_TABLA='" + parametros.getDefCampo("CVE_TABLA") + "' \n" +
							  " ) \n" +
					" ORDER BY CVE_TABLA \n";
		}
		else if (sFiltro.equals("Todos")){
			sSql = "SELECT * FROM RS_CONF_TABLA_DEPENDIENTE \n" +
					" WHERE CVE_TABLA ='" + parametros.getDefCampo("CVE_TABLA") + "' \n" +
					" ORDER BY CVE_TABLA_DEPENDIENTE \n";
		}
