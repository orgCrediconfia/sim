...
public class SimReportePagareSolidarioREP implements ReporteControlIN {
	...
	public Map getParametros(Registro parametrosCatalogo,
		...
		parametrosCatalogo.addDefCampo("ID_GRUPO", request
				.getParameter("IdGrupo"));
		parametrosCatalogo.addDefCampo("CONSULTA", "ASIGNADOS");
		LinkedList lIntegrantes = catalogoSL.getRegistros(
				"SimIntegrantesGrupo", parametrosCatalogo);
		...
		while (iteratorIntegrantes.hasNext()) {
			...
			sIntegrantes = sIntegrantes + " "
					+ (String) registro.getDefCampo("NOM_COMPLETO") + ", ";
			System.out.println(sIntegrantes);
		}

		// sIntegrantes = "Por este PagarÃ© los seÃ±or(es) firmantes, "+
		// sIntegrantes +
		// "(el (los) â€œSuscriptor(es)â€�), se obliga(n) incondicional y solidariamente en este acto a pagar a";
		...
		if (iCantidadIntegrantes <= 6) {
			...
			parametros
					.put(
							"Subreporte1",
							contextoServlet
									.getRealPath("/Reportes/Sim/prestamo/SimReportePagareSolidario_subreport0.jasper"));
			parametros
					.put(
							"Subreporte2",
							contextoServlet
									.getRealPath("/Reportes/Sim/prestamo/SimReportePagareSolidario2.jasper"));
			parametros
					.put(
							"Subreporte3",
							contextoServlet
			...
			parametros
					.put(
							"Subreporte2",
							contextoServlet
									.getRealPath("/Reportes/Sim/prestamo/SimReportePagareSolidario2.1.jasper"));
			parametros
					.put(
							"Subreporte3",
							contextoServlet
