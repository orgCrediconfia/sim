 ...
 * modificación y consulta) de los beneficiarios del seguro de vida. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimSeguroVidaBeneficiarioCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			//RECUPERA CAMPOS
			parametros.addDefCampo("ID_BENEFICIARIO",request.getParameter("IdBeneficiario"));
			registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("SimSeguroVidaBeneficiario", parametros));
			registroControl.respuesta.addDefCampo("ListaParentesco", catalogoSL.getRegistros("SimCatalogoParentesco", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/Prestamo/fSimSegVidBen.jsp?IdPrestamo="+request.getParameter("IdPrestamo")+"&IdBeneficiario="+request.getParameter("IdBeneficiario");
		}
			...
			if (request.getParameter("Filtro").equals("Alta")){
				...
				registroControl.respuesta.addDefCampo("ListaParentesco", catalogoSL.getRegistros("SimCatalogoParentesco", parametros));
				registroControl.sPagina = "/Aplicaciones/Sim/Prestamo/fSimSegVidBen.jsp?IdPrestamo="+request.getParameter("IdPrestamo")+"&IdCargoComision="+request.getParameter("IdCargoComision")+"&IdProducto="+request.getParameter("IdProducto");	
			}
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		registro.addDefCampo("ID_BENEFICIARIO", request.getParameter("IdBeneficiario"));
		...
		registro.addDefCampo("PORCENTAJE", request.getParameter("Porcentaje"));
		registro.addDefCampo("FECHA_NACIMIENTO", request.getParameter("FechaNacimiento"));
		registro.addDefCampo("ID_PARENTESCO", request.getParameter("IdParentesco"));
		
		//ACTUALIZA EL REGISTRO EN LA BASE DE DATOS
		registroControl.resultadoCatalogo = catalogoSL.modificacion("SimSeguroVidaBeneficiario", registro, iTipoOperacion);
		//INDICA A DONDE IRA AL TERMINAR LA ACTUALIZACION
		registroControl.sPagina = "/ProcesaCatalogo?Funcion=SimPrestamoCargoComisionSeguroVida&OperacionCatalogo=CR&IdCargoComision="+request.getParameter("IdCargoComision")+"&IdProducto="+request.getParameter("IdProducto")+"&IdPrestamo="+request.getParameter("IdPrestamo");
