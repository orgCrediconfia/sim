 ...
 * modificación y consulta) de los supervisores de las sucursales. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimSucursalSupervisorCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			registroControl.respuesta.addDefCampo("ListaBusqueda", catalogoSL.getRegistros("SimSucursalSupervisor", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/CatalogosGenerales/fSimSucSup.jsp?IdSucursal="+request.getParameter("IdSucursal");
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		while (lista.hasMoreElements()){
			...
			if (sNombre.startsWith("FuncionAlta")){
				...
				sIdSupervisor = sNombre.substring(11, sNombre.length());
				
				registro.addDefCampo("ID_SUCURSAL", request.getParameter("IdSucursal"));
				registro.addDefCampo("ID_SUPERVISOR", sIdSupervisor);
				
				//DA DE ALTA O BAJA LAS FUNCIONES EN LA BASE DE DATOS
				registroControl.resultadoCatalogo = catalogoSL.modificacion("SimSucursalSupervisor", registro, iTipoOperacion);
			}
		}
