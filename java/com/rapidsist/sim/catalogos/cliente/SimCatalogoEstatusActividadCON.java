 ...
 */
public class SimCatalogoEstatusActividadCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			registroControl.respuesta.addDefCampo("ListaBusqueda", catalogoSL.getRegistros("SimCatalogoEstatusActividad", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/CatalogosGenerales/fSimEstAct.jsp";
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		LinkedList listaActividadRequisito = null;
		...
		while (lista.hasMoreElements()){
			...
			if (sNombre.startsWith("FuncionAlta")){
				...
				if (listaActividadRequisito == null){
					listaActividadRequisito = new LinkedList();
				}
				sIdActividadRequisito = sNombre.substring(11, sNombre.length());
				registro.addDefCampo("ID_ETAPA_PRESTAMO", request.getParameter("IdEstatusPrestamo"));
				registro.addDefCampo("ID_ACTIVIDAD_REQUISITO", sIdActividadRequisito);
				
				//DA DE ALTA O BAJA LAS FUNCIONES EN LA BASE DE DATOS
				registroControl.resultadoCatalogo = catalogoSL.modificacion("SimCatalogoEtapaActividad", registro, iTipoOperacion);
			}
		}
		
		registroControl.sPagina = "/ProcesaCatalogo?Funcion=SimCatalogoEstatusPrestamo&OperacionCatalogo=CR&IdEstatusPrestamo="+request.getParameter("IdEstatusPrestamo");
