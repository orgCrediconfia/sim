 ...
 * modificación y consulta) de las sucursales que tiene acceso al producto. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimProductoSucursalCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			if (request.getParameter("Filtro").equals("Todos")){
				...
				if (request.getParameter("IdSucursal") != "" && !request.getParameter("IdSucursal").equals("")){
					parametros.addDefCampo("ID_SUCURSAL", request.getParameter("IdSucursal"));	
				}
			
				if (request.getParameter("NomSucursal") != "" && !request.getParameter("NomSucursal").equals("")){	
					parametros.addDefCampo("NOM_SUCURSAL", request.getParameter("NomSucursal"));
				}
				registroControl.sPagina = "/Aplicaciones/Sim/Producto/fSimProSuc.jsp?IdProducto="+request.getParameter("IdProducto");
				
			}else if (request.getParameter("Filtro").equals("Producto")){
				parametros.addDefCampo("ID_PRODUCTO",request.getParameter("IdProducto"));
				registroControl.sPagina = "/Aplicaciones/Sim/Producto/fSimProSucCon.jsp?IdProducto="+request.getParameter("IdProducto");
			}
			
			registroControl.respuesta.addDefCampo("ListaBusqueda", catalogoSL.getRegistros("SimProductoSucursal", parametros));
		}
		
		else if (iTipoOperacion == CON_CONSULTA_REGISTRO){
			/*
			//OBTIENE SOLO EL REGISTRO SOLICITADO
			parametros.addDefCampo("ID_PRODUCTO",request.getParameter("IdSucursal"));
			registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("SimProductoSucursal", parametros));
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		while (lista.hasMoreElements()){
			...
			if (sNombre.startsWith("FuncionAlta")){
				...
				registroControl.resultadoCatalogo = catalogoSL.modificacion("SimProductoSucursal", registro, iTipoOperacion);
