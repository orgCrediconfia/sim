 ...
 * modificación y consulta) de los negocios del Cliente. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimClienteNegocioCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			Registro NegocioPrincipal = new Registro ();
			NegocioPrincipal = catalogoSL.getRegistro("SimClienteNegocioPrincipal", parametros);
			String sNegocioPrincipal = (String)NegocioPrincipal.getDefCampo("B_PRINCIPAL");
			...
			registroControl.respuesta.addDefCampo("ListaTelefono", catalogoSL.getRegistros("SimNegocioTelefono", parametros));
			registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("SimClienteNegocio", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimCliNegReg.jsp?IdDomicilio="+request.getParameter("IdDomicilio")+"&NegocioPrincipal="+sNegocioPrincipal;
		}
			...
			if (request.getParameter("Filtro").equals("Alta")){
				...
				registroControl.respuesta.addDefCampo("ListaTipoNegocio", catalogoSL.getRegistros("SimCatalogoTipoNegocio", parametros));
				registroControl.respuesta.addDefCampo("ListaGiro", catalogoSL.getRegistros("SimCatalogoGiro", parametros));
				registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimCliNegReg.jsp?IdPersona="+request.getParameter("IdPersona")+"&IdExConyuge="+request.getParameter("IdExConyuge")+"&NegocioPrincipal="+sNegocioPrincipal;
			}
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		registro.addDefCampo("CVE_CLASE", request.getParameter("CveClase")!= null ? request.getParameter("CveClase") : "");
		registro.addDefCampo("FECHA_INICIO_OPERACION", request.getParameter("FechaInicioOperacion"));
		
		if(request.getParameter("BPrincipalSi")!=null){
			registro.addDefCampo("B_PRINCIPAL","V");
		}else{
			registro.addDefCampo("B_PRINCIPAL","F");
		}
		
		if (request.getParameter("IdUbicacionNegocio").equals("null")){
			registro.addDefCampo("ID_UBICACION_NEGOCIO","");
		}else {
		...
		registroControl.resultadoCatalogo = catalogoSL.modificacion("SimClienteNegocio", registro, iTipoOperacion);
		
		
		if (iTipoOperacion == 1){
			sIdNegocio = (String) registroControl.resultadoCatalogo.Resultado.getDefCampo("ID_NEGOCIO");
			sIdDomicilio = (String) registroControl.resultadoCatalogo.Resultado.getDefCampo("ID_DOMICILIO");
			registroControl.sPagina = "/ProcesaCatalogo?Funcion=SimClienteNegocio&OperacionCatalogo=CR&IdNegocio="+sIdNegocio+"&IdDomicilio="+sIdDomicilio+"&IdPersona="+request.getParameter("IdPersona")+"&IdExConyuge="+request.getParameter("IdExConyuge");
