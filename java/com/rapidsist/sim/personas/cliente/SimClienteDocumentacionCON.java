 ...
 * modificación y consulta) de la documentación del Cliente. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimClienteDocumentacionCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_TABLA){
			...
			registroControl.respuesta.addDefCampo("ListaBusqueda", catalogoSL.getRegistros("SimClienteDocumentacion", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimCliDocCon.jsp?IdPersona="+request.getParameter("IdPersona");	
		}
		else if (iTipoOperacion == CON_CONSULTA_REGISTRO){
			//RECUPERA CAMPOS
			parametros.addDefCampo("ID_DOCUMENTO",request.getParameter("IdDocumento"));
			parametros.addDefCampo("ID_PERSONA",request.getParameter("IdPersona"));
			registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("SimClienteDocumentacion", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimCliDocReg.jsp";
		}
		else if (iTipoOperacion == CatalogoControl.CON_INICIALIZACION){
			//RECUPERA CAMPOS
			if (request.getParameter("Filtro").equals("Alta")){
				parametros.addDefCampo("ID_PERSONA",request.getParameter("IdPersona"));
				registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimCliDocReg.jsp?IdPersona="+request.getParameter("IdPersona")+"&IdExConyuge="+request.getParameter("IdExConyuge");	
			}
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		registro.addDefCampo("FECHA_RECIBIDO", request.getParameter("FechaRecibido"));
		registro.addDefCampo("CVE_VERIFICADOR", request.getParameter("CveUsuarioCoordinador"));
		
		if(request.getParameter("DocCorSi")!=null){
			registro.addDefCampo("DOCUMENTO_CORRECTO","V");
		}else{
			registro.addDefCampo("DOCUMENTO_CORRECTO","F");
		}

		//ACTUALIZA EL REGISTRO EN LA BASE DE DATOS
		registroControl.resultadoCatalogo = catalogoSL.modificacion("SimClienteDocumentacion", registro, iTipoOperacion);
