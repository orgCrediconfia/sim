 ...
 * modificación y consulta) de los teléfonos de las referencias del Cliente. Esta clase es llamada por
 * el servlet {@link CatalogoS CatalogoS}.
 */
public class SimReferenciaTelefonoCON implements CatalogoControlConsultaIN, CatalogoControlActualizaIN{
	...
	public RegistroControl consulta(Registro parametros, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		if (iTipoOperacion == CON_CONSULTA_REGISTRO){
			//RECUPERA CAMPOS
			parametros.addDefCampo("IDENTIFICADOR",request.getParameter("IdReferencia"));
			...
			registroControl.respuesta.addDefCampo("registro", catalogoSL.getRegistro("SimReferenciaTelefono", parametros));
			registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimRefTel.jsp?IdReferencia="+request.getParameter("IdReferencia")+"&IdTelefono="+request.getParameter("IdTelefono");
		}
		else if (iTipoOperacion == CatalogoControl.CON_INICIALIZACION){
			//RECUPERA CAMPOS
			if (request.getParameter("Filtro").equals("Alta")){
				//parametros.addDefCampo("ID_PERSONA",request.getParameter("IdReferencia"));
				registroControl.respuesta.addDefCampo("ListaDescripcionTelefono", catalogoSL.getRegistros("SimCatalogoDescripcionTelefono", parametros));
				registroControl.sPagina = "/Aplicaciones/Sim/Personas/fSimRefTel.jsp?IdReferencia="+request.getParameter("IdReferencia")+"&IdExConyuge="+request.getParameter("IdExConyuge");	
			}
		}
	...
	public RegistroControl actualiza(Registro registro, HttpServletRequest request, HttpServletResponse response, ServletConfig config, CatalogoSL catalogoSL, Context contexto, int iTipoOperacion)throws RemoteException, Exception{
		...
		registro.addDefCampo("IDENTIFICADOR",request.getParameter("IdReferencia"));
		registro.addDefCampo("CVE_TIPO_IDENTIFICADOR", "REFERENCIA");
		...
		registroControl.resultadoCatalogo = catalogoSL.modificacion("SimReferenciaTelefono", registro, iTipoOperacion);
		//INDICA A DONDE IRA AL TERMINAR LA ACTUALIZACION
		registroControl.sPagina = "/ProcesaCatalogo?Funcion=SimClienteReferencia&OperacionCatalogo=CR&IdPersona="+request.getParameter("IdPersona")+"&IdReferencia="+request.getParameter("IdReferencia")+"&IdDomicilio="+request.getParameter("IdDomicilio")+"&IdExConyuge="+request.getParameter("IdExConyuge");
