	...
	public ResultadoCatalogo alta(Registro registro) throws SQLException{
		...
		for (int iNumParametro = 0; iNumParametro < sMontos.length; iNumParametro++) {
			...
			if (registro.getDefCampo("PRESTAMO").equals("Grupo")){
				...
				if(rs1.next()){
					//El usuario puede manipular la etapa y pasar a la sigueinte
					...
					/*
					ResultadoCatalogo resultadoCatalogoEtapaHistorico = new ResultadoCatalogo();
					SimComiteEtapaHistoricoDAO simComiteEtapaHistoricoDAO = new SimComiteEtapaHistoricoDAO();
					simComiteEtapaHistoricoDAO.setConexion(this.getConexion());
					
					resultadoCatalogoEtapaHistorico = simComiteEtapaHistoricoDAO.alta(registro);
					
					ResultadoCatalogo resultadoCatalogoAvanzarEtapa = new ResultadoCatalogo();
					SimComiteAvanzarEtapaDAO simComiteAvanzarEtapaDAO = new SimComiteAvanzarEtapaDAO();
					simComiteAvanzarEtapaDAO.setConexion(this.getConexion());
					
					resultadoCatalogoAvanzarEtapa = simComiteAvanzarEtapaDAO.alta(registro);
					*/
					//simComiteEtapaHistoricoDAO.cierraConexion();
				...
				/*
					...
					sSql = "SELECT \n"+
							...
							"AND ID_ETAPA_PRESTAMO = '" + (String)registro.getDefCampo("ID_ETAPA_PRESTAMO") + "' \n" +
							"AND ID_PRESTAMO = '" + (String)registro.getDefCampo("ID_PRESTAMO_INDIVIDUAL") + "' \n" ;
					System.out.println("solo un prestamo"+(String)registro.getDefCampo("ID_PRESTAMO_INDIVIDUAL"));
					...
					while (rs4.next()){
						...
						if (rs5.next()){
							...
							System.out.println("del prestamo");
					}
			*/
		...
		while (bBuscaFechaValida){
			...
			System.out.println("El d�a es: "+sDiaEntrega);
			...
			if (sDiaEntrega.equals("6") || sDiaEntrega.equals("7") || bDiaFestivo){
				...
				if (sOperaDomingo.equals("V") && sDiaEntrega.equals("6")){
					...
					//No se permiten d�as festivos, ni s�bados, ni domingos, por lo que obtiene el siguiente.
